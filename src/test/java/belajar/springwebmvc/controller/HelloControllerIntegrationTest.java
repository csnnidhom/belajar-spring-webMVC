...
public class HelloControllerIntegrationTest {
    ...
    @Test
    void helloNidhom() {
        ResponseEntity<String> responseEntity = restTemplate.getForEntity("http://localhost:" + port + "/hello?name=Nidhom", String.class);
        String response = responseEntity.getBody();

        Assertions.assertNotNull(response);
        Assertions.assertEquals("Hello Nidhom", response.trim());
    }
